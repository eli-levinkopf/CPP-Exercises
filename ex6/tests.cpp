...
bool test_apartment_operator_lower_than_equal_apartments() {
    // if apt1.x - apt2.x < epsilon and apt1.y - apt2.y < epsilon then they
    // are equal, and therefore neither one is lower than the other
    const double x = 123;
    const double y = -123;

    auto apt1 = Apartment({x, y});
    auto apt2 = Apartment({x, y - (EPSILON / 2)});
    auto apt3 = Apartment({x - (EPSILON / 2), y});

    return !(apt1 < apt2) && !(apt2 < apt1) &&
           !(apt1 < apt3) && !(apt3 < apt1) &&
           !(apt2 < apt3) && !(apt3 < apt2);
}
...
bool test_apartment_operator_greater_than_equal_apartments() {
    // if apt1.x - apt2.x < epsilon and apt1.y - apt2.y < epsilon then they
    // are equal, and therefore neither one is lower than the other
    const double x = 123;
    const double y = -123;
...
bool test_stack_vector_ctor() {
    std::vector<double_pair> apts{
            ...
            {4, 5}};
    Stack stack(apts);
	std::cout<< std::endl<<"top: "<<stack.top();
...
int main() {
    std::vector<TestInfo> apartment_tests = {
            ...
            TEST_CASE(test_apartment_operator_lower_than_equal_apartments),
            TEST_CASE(test_apartment_operator_greater_than),
            TEST_CASE(test_apartment_operator_greater_than_equal_apartments),
