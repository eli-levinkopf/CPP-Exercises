 ...
 * @param vec The VLA.
 * @param expected_capacity The expected _capacity.
 * @param expected_size The expected _size.
 */
...
/**
 * Asserts that the given vector has the given _size.
 * @param vec The VLVector.
 * @param expectedSize The expected _size.
 *
 * MACRO EXPANSION:
 * if (vec._capacity() != (expected_size)) {
 *      print error
 *      return 0;
 * }
 */
#define ASSERT_SIZE(vec, expected_size) if (vec.size() != (expected_size)) { \
        std::cerr << "Assertion failed: Incorrect _size. Expected: "#expected_size << "; Actual: " \
                  << vec.size() << "." << std::endl; \
        return 0; \
    }

/**
 * Asserts that the given vector has the given _capacity.
 * @param vec The VLVector.
 * @param expected_capacity The expected _capacity.
 *
 * MACRO EXPANSION:
 * if (vec._capacity() != (expected_capacity)) {
 *      print error
 *      return 0;
 * }
 */
#define ASSERT_CAPACITY(vec, expected_capacity) if (vec.capacity() != (expected_capacity)) { \
        std::cerr << "Assertion failed: Incorrect _capacity. Expected: "#expected_capacity << "; Actual: " \
