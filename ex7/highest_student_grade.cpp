...
class Student {
 public:
  ...
  Student (std::string name, double average)
	  : name_ (std::move (name)), average_ (average)
  {};
  ...
  for (size_t i = 0; i < 100; ++i)
	{
	  ...
	  double average = rand () % 100;
	  Student student (name, average);
	  students.push_back (student);
//	  std::cout << "size: " << students.size () << ", capacity: "
//				<< students.capacity () << std::endl;
	}
//  for (size_t i = 0; i < 80; ++i)
//	{
//	  students.pop_back();
//	  std::cout << "size: " << students.size () << ", capacity: "<< students.capacity () << std::endl;
//	}
//  students.erase (students.begin(), students.end()-5);
int i = 0;
