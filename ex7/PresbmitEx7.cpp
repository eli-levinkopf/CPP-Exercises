...
int TestResize() {
  ...
  ASSERT_VLA_PROPERTIES(vec, /* _capacity: */ 25,
      /* _size: */17)

  // Add another 12 items
  std::vector<int> entries(13, 1);
  vec.insert(vec.begin(), entries.cbegin(), entries.cend());
  ASSERT_VLA_PROPERTIES(vec, /* _capacity: */ 45,
      /* _size: */30)

  // Remove those 12 items (note that the _capacity won't be decreased).
  for (int i = 18; i <= 30; ++i) {
      vec.erase(vec.end() - 1);
    }
  ASSERT_VLA_PROPERTIES(vec, /* _capacity: */ 45,
  ...
  // Clear
  vec.clear();
  ASSERT_VLA_PROPERTIES(vec, /* _capacity: */ STATIC_CAP,
      /* _size: */0)

  // Adding 17 items (note that we have a _capacity of 25
...
int TestVLStringCreateString() {
  VLString<> str1;
  ASSERT_TRUE(str1._size() == 0 && str1._capacity () == STATIC_CAP)

  VLString<> str2("Hello");
  ASSERT_TRUE(str2._size() == 5 && str2._capacity () == STATIC_CAP)

  VLString<> str2_copy(str2);
  ASSERT_TRUE(str2_copy._size () == 5 && str2_copy._capacity () == STATIC_CAP)
...
int TestVLStringConcat() {
  VLString<> str;
  ASSERT_TRUE(str._size () == 0 && str._capacity () == STATIC_CAP)

  str += "Hello";
  ASSERT_TRUE(str._size () == 5 && str._capacity () == STATIC_CAP)

  str += ", World from C++";
  ASSERT_TRUE(str._size () == 21 && str._capacity () == 33)

  str += '!';
  ASSERT_TRUE(str._size () == 22 && str._capacity () == 33)
...
int TestVLStringContentStack() {
  ...
  ASSERT_TRUE(VLStringEqual(str, expected_str, /* _capacity: */ 16));
...
int TestVLStringContentHeap() {
  ...
  ASSERT_TRUE(VLStringEqual(str, expected_str, /* _capacity: */ 31));
